...
def load_ckpt(config, device, ts=None, return_ckpt=False):
    ...
    path_f = f'./checkpoints/{config.data.data}/{config.ckpt_f}.pth'

    ckpt = torch.load(path, map_location=device_id)
    ckpt_f = torch.load(path_f, map_location=device_id)
    print(f'{path} loaded')
    ...
    ckpt_dict_f= {'config': ckpt_f['model_config'], 'params_x': ckpt_f['params_x'], 'x_state_dict': ckpt_f['x_state_dict'],
                'params_adj': ckpt_f['params_adj'], 'adj_state_dict': ckpt_f['adj_state_dict']}
    #ipdb.set_trace()
