...
def get_pc_sampler(sde_x, sde_adj, shape_x, shape_adj, predictor='Euler', corrector='None', 
  ...
  def pc_sampler(model_x, model_adj, model_x_f, model_adj_f, init_flags, scaler1, scaler2):
    ...
    with torch.no_grad():
      # -------- Initial sample --------
      flags = init_flags
      x = torch.load("") # Please set your load path, or use our pre-sampled results under ./pre_data.
      adj = torch.load("") # Of course, the premise is that you first run the content of the solver_pre_data.py to generate a presampled result.
      flags = torch.load("")
