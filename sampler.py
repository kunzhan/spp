...
from utils.loader import load_ckpt, load_data, load_seed, load_device, load_model_from_ckpt, \
                         load_ema_from_ckpt, load_sampling_fn, load_eval_settings, load_batch
from utils.graph_utils import adjs_to_graphs, init_flags, quantize, quantize_mol, node_flags, mask_x, mask_adjs, gen_noise
...
class Sampler(object):
    ...
    def sample(self, scaler):
        # -------- Load checkpoint --------
        self.ckpt_dict, self.ckpt_dict_f = load_ckpt(self.config, self.device)
        self.configt = self.ckpt_dict['config']
        self.configt_f = self.ckpt_dict_f['config']
        ...
        self.log_name = f"{self.config.ckpt}-fake+s(real-fake)"
        ...
        self.model_x_f = load_model_from_ckpt(self.ckpt_dict_f['params_x'], self.ckpt_dict_f['x_state_dict'], self.device)
        self.model_adj_f = load_model_from_ckpt(self.ckpt_dict_f['params_adj'], self.ckpt_dict_f['adj_state_dict'], self.device)
        ...
        logger.log(f'scaler: {self.config.scaler}')
        ...
        for r in range(num_sampling_rounds):
            ...
            x, adj, _ = self.sampling_fn(self.model_x, self.model_adj, self.model_x_f, self.model_adj_f, self.init_flags, scaler)
...
class Sampler_mol(object):
    ...
    def sample(self, scaler1, scaler2):
        # -------- Load checkpoint --------
        self.ckpt_dict, self.ckpt_dict_f = load_ckpt(self.config, self.device)
        self.configt = self.ckpt_dict['config']
        self.configt_f = self.ckpt_dict_f['config']
        ...
        self.log_folder_name, self.log_dir, _ = set_log(self.configt, is_train=False)
        self.log_name = f"{self.config.ckpt} oc s++ steps 1.19"
        logger = Logger(str(os.path.join(self.log_dir, f'{self.log_name}.log')), mode='a')
        ...
        # -------- Generate samples --------
        logger.log(f'GEN SEED: {self.config.sample.seed}')
        logger.log(f'scaler1: {self.config.scaler1}')
        logger.log(f'scaler2: {self.config.scaler2}')
        load_seed(self.config.sample.seed)
        ...
        self.train_graph_list, _ = load_data(self.configt, get_graph_list=True)
        with open(f'data/{self.configt.data.data.lower()}_test_nx.pkl', 'rb') as f:
            self.test_graph_list = pickle.load(f)

        self.init_flags = init_flags(self.train_graph_list, self.configt, 10000).to(self.device[0])
        
        # 记录采样时间
        start_time = time.time()
        x, adj, _ = self.sampling_fn(self.model_x, self.model_adj, self.model_x_f, self.model_adj_f, self.init_flags, scaler1, scaler2)
        end_time = time.time()

        # 计算并记录耗时
        execution_time = end_time - start_time
        logger.log(f'Sampling execution time: {execution_time:.4f} seconds')
