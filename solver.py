...
class Predictor(abc.ABC):
  ...
  def __init__(self, sde, score_fn, score_fn_f, probability_flow=False):
    super().__init__()
    self.sde = sde
    # Compute the reverse SDE/ODE
    self.rsde = sde.reverse(score_fn, score_fn_f, probability_flow)
    self.score_fn = score_fn
    self.score_fn_f = score_fn_f
  @abc.abstractmethod
...
class Corrector(abc.ABC):
  ...
  def __init__(self, sde, score_fn, score_fn_f, snr, scale_eps, n_steps):
    super().__init__()
    self.sde = sde
    self.score_fn = score_fn
    self.score_fn_f = score_fn_f
...
class EulerMaruyamaPredictor(Predictor):
  ...
  def update_fn(self, x, adj, flags, is_main, t):
    dt = -1. / self.rsde.N

    if self.obj=='x':
      z = gen_noise(x, flags, sym=False)
      drift, diffusion = self.rsde.sde(x, adj, flags, t, is_main, is_adj=False)
      x_mean = x + drift * dt
      ...
      drift, diffusion = self.rsde.sde(x, adj, flags, t, is_main, is_adj=True)
...
class ReverseDiffusionPredictor(Predictor):
  def __init__(self, obj, sde, score_fn, score_fn_f, probability_flow=False):
    super().__init__(sde, score_fn, score_fn_f, probability_flow)
    self.obj = obj

  def update_fn(self, x, adj, flags, is_main, t):

    if self.obj == 'x':
      f, G = self.rsde.discretize(x, adj, flags, t, is_main, is_adj=False)
      z = gen_noise(x, flags, sym=False)
      x_mean = x - f
      ...
      f, G = self.rsde.discretize(x, adj, flags, t, is_main, is_adj=True)
      z = gen_noise(adj, flags)
      adj_mean = adj - f
...
class LangevinCorrector(Corrector):
  def __init__(self, obj, sde, score_fn, score_fn_f, snr, scale_eps, n_steps):
    super().__init__(sde, score_fn, score_fn_f, snr, scale_eps, n_steps)
    self.obj = obj

  def update_fn(self, x, adj, flags, is_main, t, scaler1, scaler2):
    sde = self.sde
    score_fn = self.score_fn
    score_fn_f = self.score_fn_f
    ...
    if self.obj == 'x':
      for i in range(n_steps):
        ...
        if not is_main:
          grad_f =  score_fn_f(x, adj, flags, t)
          grad =grad + scaler1 *( grad-grad_f)  # 1.18
          grad = grad / scaler2
...
def get_pc_sampler(sde_x, sde_adj, shape_x, shape_adj, predictor='Euler', corrector='None', 
  ...
  def pc_sampler(model_x, model_adj, model_x_f, model_adj_f, init_flags, scaler1, scaler2):
    ...
    score_fn_x_f = get_score_fn(sde_x, model_x_f, train=False, continuous=continuous)
    score_fn_adj_f = get_score_fn(sde_adj, model_adj_f, train=False, continuous=continuous)
    ...
    predictor_obj_x = predictor_fn('x', sde_x, score_fn_x, score_fn_x_f, probability_flow)
    corrector_obj_x = corrector_fn('x', sde_x, score_fn_x, score_fn_x_f, snr, scale_eps, n_steps)

    predictor_obj_adj = predictor_fn('adj', sde_adj, score_fn_adj, score_fn_adj_f, probability_flow)
    corrector_obj_adj = corrector_fn('adj', sde_adj, score_fn_adj, score_fn_adj_f, snr, scale_eps, n_steps)
    ...
    with torch.no_grad():
      ...
      for i in trange(0, 400, desc = '[Sampling]', position = 1, leave=False):
        is_main = False
        t0 = timesteps[0]
        vec_t0 = torch.ones(shape_adj[0], device=t0.device) * t0
        _x = x
        x, x_mean = corrector_obj_x.update_fn(x, adj, flags, is_main, vec_t0, scaler1, scaler2)
        adj, adj_mean = corrector_obj_adj.update_fn(_x, adj, flags, is_main, vec_t0, scaler1, scaler2)
      ...
      for i in trange(0, diff_steps, desc = '[Sampling]', position = 1, leave=False):
        ...
        if t < 0.2:
          _x = x
          x, x_mean = corrector_obj_x.update_fn(x, adj, flags, is_main, vec_t, scaler1, scaler2)
          adj, adj_mean = corrector_obj_adj.update_fn(_x, adj, flags, is_main, vec_t, scaler1, scaler2)

        _x = x
        x, x_mean = predictor_obj_x.update_fn(x, adj, flags, is_main, vec_t)
        adj, adj_mean = predictor_obj_adj.update_fn(_x, adj, flags, is_main, vec_t)
      print(' ')
...
def S4_solver(sde_x, sde_adj, shape_x, shape_adj, predictor='None', corrector='None', 
  ...
  def s4_solver(model_x, model_adj, init_flags):
    ...
    with torch.no_grad():
      ...
      timesteps = torch.linspace(sde_adj.T, eps, diff_steps, device=device)
      dt = -1. / diff_steps
      score_adj_norms_list = []
      # -------- Rverse diffusion process --------
      for i in trange(0, 100, desc = '[Sampling]', position = 1, leave=False):
        ...
        x_mean = mu_x
        adj_mean = mu_adj
        score_x0 = score_fn_x(x, adj, flags, vec_t0)
        score_adj_norm0 = torch.norm(score_x0.reshape(score_x0.shape[0], -1), dim=-1).mean()
        #print(score_adj_norm0, i)
        #if score_adj_norm0 < 27:
        #  break
      #ipdb.set_trace()
      for i in trange(0, (diff_steps), desc = '[Sampling]', position = 1, leave=False):
        t = timesteps[i]
